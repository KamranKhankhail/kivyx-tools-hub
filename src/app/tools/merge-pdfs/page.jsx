...
const PDFMergeTool = () => {
  ...
  const [fileRotations, setFileRotations] = useState({}); // { fileId: 0|90|180|270 }
  const [pageRotations, setPageRotations] = useState({}); // { globalId: 0|90|180|270 }
  ...
  const convertOfficeToPdf = async (file) => {
    ...
    page.drawText(`Original type: ${file.type}`, {
      x: 50,
      y: 700,
      size: 14,
  ...
  // Modified: handleFileSelect now stores unrotated thumbnails
  const handleFileSelect = async (e) => {
    ...
    for (const file of selected) {
      ...
      if (file.type === "application/pdf") {
        ...
        // Store original, unrotated image preview
        ...
        const newFile = {
          ...
          pages: 1,
          pageOrder: [1],
          thumbnail: originalPreview,
          type: file.type,
          pdfDoc: null,
          loading: false,
          pageThumbnails: { 1: originalPreview },
        };
        setFiles((p) => [...p, newFile]);
        setGlobalPageOrder((p) => [
          ...p,
          { fileId: tempId, originalPageNum: 1, globalId: `${tempId}-1` },
        ]);
      } else {
        const converting = {
          id: tempId,
        ...
        setFiles((p) => [...p, converting]);
        try {
          const converted = await convertOfficeToPdf(file);
          // Generate unrotated thumbnail for converted PDF
          const { thumbnail, pageCount, pdfDoc } = await generatePdfThumbnail(
            converted
          );
          const pageOrder = Array.from({ length: pageCount }, (_, i) => i + 1);
        ...
        <div className="border-2 border-dashed border-[#1fd5e9] rounded-[22px] min-h-[100vh] bg-white p-6 flex flex-wrap gap-8 items-start">
          {files.length === 0 ? (
            <div className="w-full flex flex-col items-center justify-center py-16">
              ...
              <p className="text-[#4a5565] mb-[20px] text-xl mt-[20px]">
                Add <strong>PDF, image, Word, Excel</strong>, and{" "}
                <strong>PowerPoint</strong> files
              </p>
              <div className="flex gap-2 flex-wrap justify-center">
                {["PDF", "DOC", "XLS", "PPT", "PNG", "JPG"].map((f) => (
                  <span
                    key={f}
