...
//       else if (from === "min/mi") kmh = 96.5606 / value;
...
//       if (from === "°C") celsius = value;
//       else if (from === "°F") celsius = ((value - 32) * 5) / 9;
//       else if (from === "Gas Mark") celsius = value * 14 + 121;

//       if (to === "°C") return celsius;
//       if (to === "°F") return (celsius * 9) / 5 + 32;
//       if (to === "Gas Mark") return (celsius - 121) / 14;
//       return value;
//     },
//   },
//   "Engine Power": {
...
//     "Radiation Units",
//   ],
//   "Cooking & Food": ["Cooking Volume", "Cooking Weight", "Oven Temperature"],
//   "Health & Fitness": ["Pace / Running Speed"],
//   Miscellaneous: ["Engine Power", "Shoe Size", "Clothing Size"],
// };
...
//   const [expandedGroups, setExpandedGroups] = useState({ "Core Daily": true });

//   const currentConverter = converters[selectedCategory];
//   const numValue = parseFloat(value) || 0;
//   const mainResult = convertValue(numValue, fromUnit, toUnit, selectedCategory);

//   const allResults = currentConverter.units.map((unit) => ({
//     unit,
...
//         if (conv.units.includes(from) && conv.units.includes(to)) {
...
//                               : "transparent",
//                           color:
//                             selectedCategory === category ? "#fff" : "#4B5563",
//                           border: "none",
//                           borderRadius: "8px",
//                           cursor: "pointer",
//                           textAlign: "left",
...
//               >
//                 <ArrowRightLeft size={20} color="#1E3A8A" />
//               </button>
//             </div>
...
//               </div>
//               <div style={{ fontSize: "18px", opacity: 0.9 }}>{toUnit}</div>
//             </div>
//           </div>
//         </div>
...
const converters = {
  ...
  "Number System Converter": {
    ...
    convert: (value, from, to) => {
      let decValue;
      try {
        if (from === "Decimal") decValue = parseInt(value, 10);
        else if (from === "Binary") decValue = parseInt(value, 2);
        else if (from === "Hexadecimal") decValue = parseInt(value, 16);
        else if (from === "Octal") decValue = parseInt(value, 8);
        else return NaN;

        if (isNaN(decValue)) return "Invalid input value";

        if (to === "Decimal") return decValue.toString(10);
        if (to === "Binary") return decValue.toString(2);
        if (to === "Hexadecimal") return decValue.toString(16).toUpperCase();
        if (to === "Octal") return decValue.toString(8);
        return NaN;
      } catch (error) {
  ...
  "Roman Numeral Converter": {
    ...
    convert: (value, from, to) => {
      ...
      if (from === "Decimal" && to === "Roman")
        return toRoman(parseInt(value, 10));
      if (from === "Roman" && to === "Decimal")
        return fromRoman(value.toUpperCase());
      return NaN;
    },
  },
};
  ...
  switch (categoryName) {
    case "File Size Calculator": {
      ...
      if (!isNaN(duration) && !isNaN(bitrate) && duration > 0 && bitrate > 0) {
        return `${(duration * bitrate).toFixed(0)} bits`;
      } else if (!isNaN(size) && !isNaN(bitrate) && size > 0 && bitrate > 0) {
        return `${(size / bitrate).toFixed(2)} seconds`;
      } else if (!isNaN(size) && !isNaN(duration) && size > 0 && duration > 0) {
        return `${(size / duration).toFixed(2)} bps`;
      }
    ...
    case "Body Fat Calculator": {
      ...
      return `Body Fat: ${bodyFatPercentage.toFixed(2)}% (Approx)`;
    }
    case "Calorie Burn Calculator": {
      ...
      let metValue = 1.0;
      if (activityType.includes("running")) metValue = 8.0;
      else if (activityType.includes("walking")) metValue = 3.5;
      else if (activityType.includes("cycling")) metValue = 7.5;
      else if (activityType.includes("swimming")) metValue = 6.0;
      else if (activityType.includes("aerobics")) metValue = 5.0;
      else if (activityType.includes("sitting")) metValue = 1.5;
    ...
    case "Water Intake Calculator": {
      ...
      if (activityLevel.includes("medium")) recommendedIntakeMl *= 1.1;
      else if (activityLevel.includes("high")) recommendedIntakeMl *= 1.25;
          ...
          >
                ...
                {currentConverter.inputs.map((input) => (
                  <div key={input.name} style={{ marginBottom: "20px" }}>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleCalculatorInputChange(input.name, e.target.value)
                      }
                      style={{
                        width: "100%",
                        padding: "12px 16px",
            ...
            >
              ...
              >
                {currentConverter.type === "unit"
                  ? formatNumber(mainResult)
                  : typeof calculatorResult === "string" &&
                    calculatorResult.includes("\n")
                  ? calculatorResult
                      .split("\n")
                      .map((line, index) => <div key={index}>{line}</div>)
                  : formatNumber(calculatorResult)}
              </div>
